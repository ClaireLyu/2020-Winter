...
```{r}
admission %>% 
  ggplot(mapping = aes(x = LANGUAGE, y = factor(1), fill = LANGUAGE)) + 
  geom_count() + 
  labs(y = "Language")
```
...
Link 'D_ITEMS.csv' to 'CHARTEVENTS.csv' by key "ITEMID": 
```{r}
cd <- left_join(chartevent, ditem, by = "ITEMID")
```

Find the potential "ITEMID" that contains systolic blood pressure:
```{r}
unique(subset(cd$ITEMID, str_detect(cd$LABEL, "systolic")))
```