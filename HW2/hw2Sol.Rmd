...
admission %>%
  ...
  labs(x = "Admission Month",
       ...
       subtitle = "We see the number of hospital admissions are 
...
admission %>% 
  ...
  labs(x = "Admission weekday",
       ...
       subtitle = "Weekdays are busier than weekends")
```
...
admission %>% 
  ...
  labs(x = "Admission hour",
       ...
       subtitle = "The peak hour of admissions is 7 a.m. in the morning. 
       And there are commonly more admissions from 12 p.m. throughout midnight. ")
```

Further question to consider: Why those peaks?  
```{r}
admission %>% 
...
admission %>% 
  ...
  labs(x = "Length of hospital stay (days)",
       ...
       subtitle = "The majority of patients stayed in the hospital less than 50 days.")
```
...
admission %>% 
  ...
  labs(x = "Number of admissions per patient",
       ...
       subtitle = "Most patients have only admitted to the hospital once.")
```
...
For those variables, we need to summarize them based on only unique patients.
...
admission %>% 
  distinct(SUBJECT_ID, .keep_all = TRUE) %>%
  ggplot(mapping = aes(x = HOSPITAL_EXPIRE_FLAG, fill = HOSPITAL_EXPIRE_FLAG)) + 
  geom_bar() + 
  ...
  labs(title = "Number of Deaths vs. Survivals",
       subtitle = "The majority of patients survived after their admissions 
       to the hospital.")
```
...
Next, we summerize some variables based on unique patients using appropriate graphs.
...
aP %>% 
  ...
  coord_polar("y") +
  labs(y = "Gender",
       title = "Distribution of Patients' Gender")
```
...
aP %>% 
  ...
  labs(x = "Age at Admission",
       ...
       subtitle = "A large proportion of admissions comes from Newborns.
       No admission from age 1 to 13.",
       caption = "Those elder than 300 are those who are older than 89 years old at 
       any time in the database have had their date of birth shifted to obscure their 
       age and comply with HIPAA.")
```
...
iP %>% 
  ...
  coord_polar("y") +
  labs(y = "Gender",
       title = "Distribution of ICU Patients' Gender")
```
...
First, I grouped the patients by their SUBJECT_ID and ranked their time transfered into ICU from the earliest to the latest. Then, I extracted the earliest time, which represents their first time staying in ICU. 
```{r}
ICU %>%
  group_by(SUBJECT_ID) %>%
  arrange(INTIME) %>%
