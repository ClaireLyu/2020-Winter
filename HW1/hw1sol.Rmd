...
**Solution** There are 46520 unique patients.
    ...
    ```
**Solution** Below are the lists of all possible values taken by variable `INSURANCE`, `LANGUAGE`, `RELIGION`, `MARITAL_STATUS`, and `ETHNICITY`.
    ```{bash}
...
**Solution** A number with a dollar sign ahead refers to the location of the variable in the command line. "$1" refers to 'pride_and_prejudice.txt'; "$2" refers to '20'; and "$3" refers to '5'. So, in all, we first select the first 20 lines from 'pride_and_prejudice.txt' and from those 20 lines, we select the last 5 lines.
We need the first line of the shell script to inform that we are using 'sh' as shell. 
...
**Solution** The following is my output of calling runSim.R by using `rep = 50`, and `seed = 203`.
    ```{bash}
    Rscript autoSim.R
    ```
