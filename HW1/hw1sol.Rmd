...
**Solution** There are 46520 unique patients.
    ...
    ```
**Solution** Below are the lists of all possible values taken by variable `INSURANCE`, `LANGUAGE`, `RELIGION`, `MARITAL_STATUS`, and `ETHNICITY` in the order.
    ```{bash}
...
**Solution** Below are the R script that collects results from output files and the resulting table.
    ```{bash}
    cat datClt.R
    Rscript datClt.R
    ```
