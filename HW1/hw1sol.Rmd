...
**Solution** There are 58977 rows in this data file.
    ...
    ```
**Solution** There are 47106 unique patients.
    ```{bash}
    awk -F, '{ print $2 }' /home/203bdata/mimic-iii/ADMISSIONS.csv | tail -n+2 | uniq | wc -l
    ```
**Solution** Below are the lists of all possible values taken by variable `INSURANCE`, `LANGUAGE`, `RELIGION`, `MARITAL_STATUS`, and `ETHNICITY`.
    ...
    ```
**Solution** There are 2128 Hispanic patients. 
    ```{bash}
    awk -F, '{ print $14 }' /home/203bdata/mimic-iii/ADMISSIONS.csv | tail -n+2 | sort | grep 'HISPANIC' | wc -l
    ```
...
2. What's the difference between the following two commands?
    ...
    ```
**Solution** The first command directs the output string 'hello,world' into a new file named 'test1.txt'. The second command appends the string 'hello,world' into the existing file named 'test2.txt'.
