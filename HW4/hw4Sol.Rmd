...
A histogram below shows that the majority was between 40 to 90 years old.
```{r}
study_admissions %>% 
  collect() %>%
  ...
  labs(x = "Age", title = "Distribution of Age among Patients")
```

There were approximately 25% more males than females among the patients.
```{r}
study_admissions %>% 
  collect() %>%
...
```
The majority of the patients were White. The proportion of Asian, Black, Hispanic and unknown ethnicity did not differ a lot from each other.
```{r}
study_admissions %>% 
  collect() %>%
  distinct(subject_id, .keep_all = TRUE) %>%
...
```
Nearly two thirds of the patients who had malignant neoplasm died within 30 days after discharged.
```{r}
study_admissions %>% 
  collect() %>%
  distinct(subject_id, .keep_all = TRUE) %>%
...
```
Among those who had malignant neoplasm, it was the principal diagnosis about 60%.
```{r}
study_admissions %>% 
  collect() %>%
...
```
The severity of level 1 and 3 was more than that of level 2 and 4.
```{r}
study_admissions %>% 
  collect() %>%
...
```
The histogram below shows the top 10 actual code corresponding to the diagnosis assigned to the patients.
```{r}
study_admissions %>% 
  collect() %>%
  group_by(icd9_code) %>%
