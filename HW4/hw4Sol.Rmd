...
study_admissions %>% 
  ...
  labs(x = "Age", title = "Patient's Age")
```
...
study_admissions %>% 
  ...
  theme(axis.text.x=element_blank(),
        ...
        axis.title.x=element_blank()) +
  geom_text(stat = 'count', aes(label = ..count.., vjust = -0.2)) +
  labs(title = "Distribution of Mortality among Patients")
```
```{r}
study_admissions %>% 
  collect() %>%
  ...
  theme(axis.text.x=element_blank(),
        ...
        axis.title.x=element_blank()) +
  geom_text(stat = 'count', aes(label = ..count.., vjust = -0.2)) +
  labs(title = "Distribution of Principal among Admissions")
```
```{r}
study_admissions %>% 
  collect() %>%
  ggplot(mapping = aes(x = drg_severity, 
                       fill = drg_severity)) + 
  geom_bar() + 
  labs(title = "Distribution of Severity among Admissions")
```
...
## Analytics
```{r}
model <- glm(mortality ~ age + mortality + drg_severity + icd9_code
             + principal_dx + gender + ethnic_group, data = study_admissions)
summary(model)
